...
interface ApplicationPermissions {
  ...
  spacesPermissions: { [key: string]: string };
}
...
export class AuthorizationService {
  ...
  constructor(
    @Inject(InjectionTokens.CacheService) private readonly cacheService: ICacheService,
    ...
    if (cachedValue === null) {
      ...
      if (cachedValue === null) {
        try {
          ...
          cacheKey,
          isAuthorized ?
            this.authorizedCacheDurationInSeconds : this.unauthorizedTicketCacheDurationInSeconds,
  ...
  public async isAuthorizedThroughS2S(
    ubiAppId: string,
    ubiSessionId: string,
    ticket: string)
    : Promise<boolean> {
  ...
  public async getApplicationPermissions(
    ...
    this.logger.log('AuthorizationService', `Checking Application ${ubiAppId} Permissions`);
    // tslint:disable-next-line:max-line-length
    const url: string = `${this.configService.config.s2sTicketUrl}/v2/applications/${ubiAppId}/permissions`;
    const response = await request.get(url)
      .set(AuthenticationHeaders.UbiAppId, ubiAppId)
      .set(AuthenticationHeaders.Authorization, `${UbiServicesTicketScheme.S2S} t=${ticket}`)
  ...
  public async isAuthorizedThroughAdmin(
    ...
    try {
      ...
      const { platformType }: UbiservicesProfile = await
       this.getUbiservicesProfile(ubiAppId, ubiSessionId, ticket);
  ...
  public async getUbiservicesProfile(
    ...
    const response = await request
      ...
      .set(AuthenticationHeaders.UbiAppId, ubiAppId)
      .set(AuthenticationHeaders.Authorization, `${UbiServicesTicketScheme.Admin} t=${ticket}`)
      .set(AuthenticationHeaders.UbiSessionId, ubiSessionId);
